...
int main() {
    ...
    // --- Polymorphism demo: store all in a vector of Vehicle* ---
    ...
    // Attempting direct access to private members would produce a compile error.
    // We can only interact through public getters and methods.
    ...
    // drain_battery until depleted, then catch BatteryDepletedError
    try {
    ...
    // charge_battery without being in charging state
    try {
    ...
    // Catch all drone exceptions generically
    try {
    ...
    while (!auto_drone.mission_complete()) {
        try {
            ...
            // Simulate a high-severity obstacle at waypoint 3
            if (wp_count == 3) {
                ...
                // Recover: take off again to continue
